...
class LaserBoard:
    def __init__(self, height, width, src = 1):
    ...
    def pos_tracking_demo(self):
        ...
        def vectf(X):
            ...
            f = np.zeros(9)
            f[0] = -(AP*(X[0]/(np.sqrt(X[0]*X[0] + X[1]*X[1] + X[2]*X[2]))) -
                     BP*(X[3]/(np.sqrt(X[3]*X[3] + X[4]*X[4]  + X[5]*X[5])))) + (a[0] - b[0])
            f[1] = -(AP*(X[1]/(np.sqrt(X[0]*X[0] + X[1]*X[1] + X[2]*X[2]))) -
                     BP*(X[4]/(np.sqrt(X[3]*X[3] + X[4]*X[4]  + X[5]*X[5])))) + (a[1] - b[1])
            f[2] = -(AP*(X[2]/(np.sqrt(X[0]*X[0] + X[1]*X[1] + X[2]*X[2]))) -
                     BP*(X[5]/(np.sqrt(X[3]*X[3] + X[4]*X[4]  + X[5]*X[5]))))
            f[3] = -(AP*(X[0]/(np.sqrt(X[0]*X[0] + X[1]*X[1] + X[2]*X[2]))) -
                     CP*(X[6]/(np.sqrt(X[6]*X[6] + X[7]*X[7]  + X[8]*X[8])))) + (a[0] - c[0])
            f[4] = -(AP*(X[1]/(np.sqrt(X[0]*X[0] + X[1]*X[1] + X[2]*X[2]))) -
                     CP*(X[7]/(np.sqrt(X[6]*X[6] + X[7]*X[7]  + X[8]*X[8])))) + (a[1] - c[1])
            f[5] = -(AP*(X[2]/(np.sqrt(X[0]*X[0] + X[1]*X[1] + X[2]*X[2]))) -
                     CP*(X[8]/(np.sqrt(X[6]*X[6] + X[7]*X[7]  + X[8]*X[8]))))
            f[6] = -(CP*(X[6]/(np.sqrt(X[6]*X[6] + X[7]*X[7] + X[8]*X[8]))) -
                     BP*(X[3]/(np.sqrt(X[3]*X[3] + X[4]*X[4]  + X[5]*X[5])))) + (c[0] - b[0])
            f[7] = -(CP*(X[7]/(np.sqrt(X[6]*X[6] + X[7]*X[7] + X[8]*X[8]))) -
                     BP*(X[4]/(np.sqrt(X[3]*X[3] + X[4]*X[4]  + X[5]*X[5])))) + (c[1] - b[1])
            f[8] = -(CP*(X[8]/(np.sqrt(X[6]*X[6] + X[7]*X[7] + X[8]*X[8]))) -
                     BP*(X[5]/(np.sqrt(X[3]*X[3] + X[4]*X[4]  + X[5]*X[5]))))
            return f
...
##                    point = key_points[i].pt
##                    x = '%.1f' % point[0]
##                    y = '%.1f' % point[1]
##                    diameter = key_points[i].size
##                    print("Blob " + str(i) + " detected at ( " + str(x) + " , " + str(y) + ")" + "size = "
##                          + str(diameter) + " pixels wide", (255, 0, 0), cv2.DRAW_MATCHES_FLAGS_DRAW_RICH_KEYPOINTS)
                A = key_points[0].pt
                B = key_points[1].pt
                C = key_points[2].pt 
                A = np.array([A[0], A[1] + 600 ,0])
                B = np.array([B[0], B[1] + 600 ,0])
                C = np.array([C[0], C[1] + 600 ,0])
